...
class TestRAGSystemIntegration:
    ...
    def test_rag_system_initialization(
        self, mock_anthropic, mock_embedding, mock_chroma, test_config
    ):
    ...
    @patch(
        "rag_system.chromadb.utils.embedding_functions.SentenceTransformerEmbeddingFunction"
    )
    ...
    @patch("ai_generator.anthropic.Anthropic")
    def test_query_with_max_results_zero_bug(
        self,
        mock_anthropic,
        mock_embedding,
        mock_chroma,
        test_config_with_zero_results,
        mock_tool_use_response,
    ):
    ...
    def test_query_with_session_management(
        ...
        response1, sources1 = rag_system.query(
            "First question", session_id="test-session"
        )
...
Linear regression is used for predictive modeling."""
        ...
        try:
            ...
            with patch.object(
                rag_system.document_processor, "process_course_document"
            ) as mock_processor:
                ...
                )  # Once for metadata, once for content
        finally:
            os.unlink(temp_file)
