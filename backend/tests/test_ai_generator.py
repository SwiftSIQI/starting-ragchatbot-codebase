...
class TestAIGenerator:
    ...
    def ai_generator(self, mock_anthropic_client):
        ...
        with patch(
            ...
            return AIGenerator(
                api_key="test-key", model="test-model", base_url="https://test-api.com"
            )
    ...
    def test_generate_response_without_tools(
        self, ai_generator, mock_anthropic_client, mock_text_response
    ):
    ...
    def test_generate_response_with_tools_no_tool_use(
        self, ai_generator, mock_anthropic_client, mock_text_response, mock_tool_manager
    ):
    ...
    def test_handle_sequential_tool_execution_workflow(
        self,
        ai_generator,
        mock_anthropic_client,
        mock_tool_use_response,
    ...
    def test_handle_multiple_tool_calls(
        ...
        result = ai_generator._handle_tool_execution(
            mock_multi_tool_response, base_params, mock_tool_manager
        )
    ...
    def test_error_handling_in_tool_execution(
        ...
        assert any(
            "Tool execution error" in str(result["content"])
            for result in tool_result_message
        )
    ...
    def test_api_call_structure_without_history(
        self, ai_generator, mock_anthropic_client, mock_text_response
    ):
    ...
    def test_api_call_structure_with_history(
        self, ai_generator, mock_anthropic_client, mock_text_response
    ):
    ...
    def test_sequential_tool_execution_single_round(
        self, ai_generator, mock_anthropic_client, mock_tool_manager
    ):
        ...
        result = ai_generator._handle_sequential_tool_execution(
            mock_tool_response, base_params, mock_tool_manager
        )
    ...
    def test_sequential_tool_execution_two_rounds(
        ...
        base_params = {
            "model": "test-model",
            "messages": [
                {"role": "user", "content": "What is in lesson 3 of Python Course?"}
            ],
            "system": "system",
            "tools": [
        ...
        result = ai_generator._handle_sequential_tool_execution(
            mock_first_response, base_params, mock_tool_manager, max_rounds=2
        )
        ...
        mock_tool_manager.execute_tool.assert_any_call(
            "search_course_content", query="lesson 3", course_filter="Python Course"
        )
    ...
    def test_sequential_tool_execution_max_rounds_limit(
        ...
        result = ai_generator._handle_sequential_tool_execution(
            mock_tool_response, base_params, mock_tool_manager, max_rounds=2
        )
    ...
    def test_sequential_tool_execution_api_error(
        self, ai_generator, mock_anthropic_client, mock_tool_manager
    ):
    ...
    def test_fallback_to_legacy_execution(
        self, ai_generator, mock_anthropic_client, mock_tool_manager
    ):
        ...
        ai_generator._handle_sequential_tool_execution = Mock(
            side_effect=failing_sequential
        )
